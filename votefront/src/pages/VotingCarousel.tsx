...
export default function VotingCarousel({ categories, onSubmit }: Props) {
  ...
  if (isReviewing) {
    return (
      <div className="max-w-md mx-auto p-4 pb-24 min-h-screen bg-[#FCFBF8] pt-8">
        <h2 className="text-4xl font-['Cormorant_Infant'] text-red-900 mb-6 text-center">Review your vote</h2>
        <div className="space-y-4">
          {categories.map((cat) => {
            const selectedNominee = selections[cat.category_id];
            return (
              <div key={cat.category_id} className="bg-white p-4 rounded-2xl shadow-sm border border-stone-200 flex items-center gap-4">
                <img 
                  src={selectedNominee?.photo_url || '/img/dummy.jpg'} 
                  alt={selectedNominee?.name || 'Nominee'}
                  className="w-16 h-16 rounded-xl object-cover shrink-0 border border-stone-200 shadow-sm"
                />
                <div className="flex-1">
                  <p className="text-xs text-amber-600 uppercase font-bold tracking-wider">{cat.category_name}</p>
                  <p className="text-lg font-bold text-stone-800 leading-snug">{selectedNominee?.name}</p>
                </div>
                <button 
                  onClick={() => { 
                  ...
                  className="text-red-900 text-sm font-bold px-4 py-2 bg-red-50 rounded-lg active:bg-red-100 border border-red-100"
                >
                  Edit
                </button>
              </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-stone-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          <div className="max-w-md mx-auto">
            <button 
              ...
              className="w-full bg-red-900 text-amber-500 py-4 rounded-2xl font-black text-xl shadow-lg shadow-red-900/20 active:scale-95 transition-transform"
            >
              Cast My Vote
            </button>
          </div>
  ...
  // --- VOTING STEP SCREEN ---
  return (
    <div className="max-w-md mx-auto p-4 flex flex-col min-h-screen bg-[#FCFBF8] pt-8">
      <div className="mb-8">
      ...
      <h2 className="text-3xl font-black text-stone-800 mb-8 leading-tight">
        ...
        {currentCategory.nominees.map((nominee) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left p-4 rounded-2xl border-2 transition-all duration-200 flex items-center gap-4 shadow-sm ${
                isSelected
            ...
            >
              <img 
                ...
                alt={nominee.name}
                className={`w-24 h-24 rounded-2xl object-cover shrink-0 shadow-sm transition-all duration-200 ${
                  isSelected ? 'border-2 border-amber-500' : 'border border-stone-100'
                }`}
              />
              <div className="flex-1 py-1">
                <span className={`text-lg font-bold leading-snug block ${
      ...
      <div className="mt-8 mb-6 flex justify-between gap-4">
        {isEditingFromReview ? (
          <button 
            ...
            className="w-full py-4 bg-stone-200 text-stone-700 rounded-xl font-bold active:bg-stone-300 transition-colors"
          >
            Cancel Edit
          </button>
        ) : (
          <>
            <button 
              ...
              className="px-6 py-4 rounded-xl font-bold text-stone-400 disabled:opacity-0 active:bg-stone-200 transition-colors"
            >
              ← Back
            </button>
            {selections[currentCategory.category_id] && (
               ...
               className="px-8 py-4 bg-red-900 text-amber-500 rounded-xl font-bold shadow-lg active:scale-95 transition-transform"
             >
