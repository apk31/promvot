...
type VoteStatus = 'loading' | 'error' | 'welcome' | 'confirm' | 'voting' | 'success' | 'already_voted' | 'before_start' | 'after_end';
const WIB_TIMEZONE = 'Asia/Jakarta';
const votingWindowFormatter = new Intl.DateTimeFormat('id-ID', {
...
export default function VotePage() {
  ...
  const [timingInfo, setTimingInfo] = useState<{startTime: string} | null>(null);
  ...
  useEffect(() => {
    switch (status) {
      ...
      case 'already_voted':
        ...
        break;
      case 'error':
        document.title = "Error | Promnight 2026 Vote Area";
        break;
      case 'before_start':
        document.title = "Voting Not Open | Promnight 2026";
        break;
      case 'after_end':
        document.title = "Voting Closed | Promnight 2026";
        break;
      default:
  ...
  useEffect(() => {
    const checkStatusAndToken = async () => {
      try {
        // 1. Check if the voting window is open
        const statusRes = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/voting-status`);
        const statusData = await statusRes.json();

        if (!statusRes.ok) {
          setErrorMessage(statusData.error || 'Voting schedule is unavailable.');
          setStatus('error');
          return;
        }

        if (statusData.isBeforeStart) {
          setTimingInfo({ startTime: statusData.startTime });
          setStatus('before_start');
          return;
        }

        if (statusData.isAfterEnd) {
          setStatus('after_end');
          return;
        }

        // 2. If window is open, proceed to token verification
        if (token) {
          verifyToken();
        } else {
    ...
    const verifyToken = async () => {
      try {
        ...
        // Handle middleware rejections just in case
        if (response.status === 403 && data.message === "Voting has not started yet.") {
            setStatus('before_start');
            return;
        }
        if (response.status === 403 && data.message === "Voting has already closed.") {
            setStatus('after_end');
            return;
        }
  ...
  const handleVoteSubmit = async (finalVotes: VoteSelection[]) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        // Handle middleware rejection during vote submission
        if (response.status === 403 && data.message === "Voting has already closed.") {
            setStatus('after_end');
            return;
        }
  ...
  // --- THEME FIX: BEFORE START SCREEN (ENVELOPE) ---
  if (status === 'before_start') {
    const startDate = timingInfo
      ? `${votingWindowFormatter.format(new Date(timingInfo.startTime))} WIB`
      : 'soon';
      ...
      <div className="min-h-screen bg-[#e4dcc2] flex items-center justify-center p-4">
        <div className="relative bg-white max-w-md w-full rounded-3xl shadow-xl text-center border border-stone-200 pt-[150px] pb-10 px-8 mt-12">
          ...
          <div className="absolute inset-0 pointer-events-none rounded-3xl overflow-hidden">
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute top-0 left-0 w-full h-[120px] drop-shadow-[0_4px_4px_rgba(0,0,0,0.05)]">
          ...
          <div className="relative z-10"><br />
            <h1 className="text-3xl font-['Cormorant_Infant'] font-bold text-stone-800 mb-2">Not Open Yet</h1>
            <p className="text-stone-500 mb-8 font-medium">
              The voting booth is currently locked. It will open on:
            </p>
            <div className="bg-stone-100 py-3 px-4 rounded-xl mb-8">
              <p className="text-stone-800 font-bold">{startDate}</p>
            </div>
            <button 
              onClick={() => window.location.replace('/')}
              className="w-full bg-stone-200 text-stone-700 py-4 rounded-xl font-bold text-lg hover:bg-stone-300 active:bg-stone-400 transition-colors"
            >
              Return Home
            </button>
          </div>
  ...
  // --- THEME FIX: AFTER END SCREEN (ENVELOPE) ---
  if (status === 'after_end') {
    return (
      <div className="min-h-screen bg-[#e4dcc2] flex items-center justify-center p-4">
        <div className="relative bg-white max-w-md w-full rounded-3xl shadow-xl text-center border border-stone-200 pt-[150px] pb-10 px-8 mt-12">
          ...
          <div className="relative z-10"><br />
            <h1 className="text-3xl font-['Cormorant_Infant'] font-bold text-stone-800 mb-2">Voting Closed</h1>
            <p className="text-stone-500 mb-8 font-medium">
              The voting session for Promnight 2026 has officially concluded.
            </p>
